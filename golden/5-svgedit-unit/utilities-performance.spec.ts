...
test.describe('utilities performance', function () {
  let currentLayer;
  let groupWithMatrixTransform;
  let textWithMatrixTransform;
  test.beforeEach(async ({ page }) => {
    ...
    const newNode = document.body.ownerDocument.importNode(
      editor.documentElement,
      true
    );
    document.body.append(newNode);

    currentLayer = document.getElementById('layer1');
    groupWithMatrixTransform = document.getElementById(
      'svg_group_with_matrix_transform'
    );
    textWithMatrixTransform = document.getElementById(
      'svg_text_with_matrix_transform'
    );
  });
  ...
  test('Test svgCanvas.getStrokedBBox() performance with matrix transforms', async function ({
    page,
  }) {
    ...
    assert.isBelow(
      ave,
      20,
      'svgedit.utilities.getStrokedBBox average execution time is less than 20 ms'
    );
    console.log(
      'Pass1 svgCanvas.getStrokedBBox total ms ' +
        total +
        ', ave ms ' +
        ave.toFixed(1) +
        ',\t min/max ' +
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(function () {
        ...
        assert.isBelow(
          avg,
          2,
          'svgedit.utilities.getStrokedBBox average execution time is less than 1 ms'
        );
        console.log(
          'Pass2 svgCanvas.getStrokedBBox total ms ' +
            total +
            ', ave ms ' +
            avg.toFixed(1) +
            ',\t min/max ' +
