...
test.describe('utilities', function () {
  /**
  ...
  test('Test svgedit.utilities package', async function ({ page }) {
    assert.ok(utilities);
    assert.ok(utilities.toXml);
    assert.equal(
      typeof utilities.toXml,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test svgedit.utilities.toXml() function', async function ({ page }) {
  ...
  test('Test svgedit.utilities.encode64() function', async function ({ page }) {
    ...
    assert.equal(
      encode64('`~!@#$%^&*()-_=+[{]}\\|;:\'",<.>/?'),
      'YH4hQCMkJV4mKigpLV89K1t7XX1cfDs6JyIsPC4+Lz8='
    );
  });

  test('Test svgedit.utilities.decode64() function', async function ({ page }) {
    ...
    assert.equal(
      decode64('YH4hQCMkJV4mKigpLV89K1t7XX1cfDs6JyIsPC4+Lz8='),
      '`~!@#$%^&*()-_=+[{]}\\|;:\'",<.>/?'
    );
  });

  test('Test svgedit.utilities.convertToXMLReferences() function', async function ({
    page,
  }) {
  ...
  test('Test svgedit.utilities.bboxToObj() function', async function ({
    page,
  }) {
  ...
  test('Test getUrlFromAttr', async function ({ page }) {
    assert.equal(utilities.getUrlFromAttr('url(#foo)'), '#foo');
    assert.equal(
      utilities.getUrlFromAttr('url(somefile.svg#foo)'),
      'somefile.svg#foo'
    );
    assert.equal(utilities.getUrlFromAttr('url("#foo")'), '#foo');
    assert.equal(utilities.getUrlFromAttr('url("#foo")'), '#foo');
  ...
  test('Test getPathDFromSegments', async function ({ page }) {
  ...
  test('Test getPathDFromElement', async function ({ page }) {
    ...
    svgroot.append(elem);
    const closeEnough =
      /M0,4 C0,2.3\d* 0.9\d*,1 2,1 L8,1 C9.0\d*,1 10,2.3\d* 10,4 L10,9 C10,10.6\d* 9.0\d*,12 8,12 L2,12 C0.9\d*,12 0,10.6\d* 0,9 L0,4 Z/;
    ...
    assert.equal(
      ...
      'M5,11 C5,5.475138121546961 7.237569060773481,1 10,1 C12.762430939226519,1 15,5.475138121546961 15,11 C15,16.524861878453038 12.762430939226519,21 10,21 C7.237569060773481,21 5,16.524861878453038 5,11 Z'
    );
    elem.remove();
    ...
    assert.equal(
      getPathDFromElement({ tagName: 'something unknown' }),
      undefined
    );
  });

  test('Test getBBoxOfElementAsPath', async function ({ page }) {
    /**
    ...
    function getBBoxOfElementAsPath(elem, addSVGElementsFromJson, pathActions) {
      const bbox = utilities.getBBoxOfElementAsPath(
        elem,
    ...
    svgroot.append(elem);
    let bbox = getBBoxOfElementAsPath(
      elem,
      mockaddSVGElementsFromJson,
      mockPathActions
    );
  ...
  test('Test convertToPath rect', async function ({ page }) {
  ...
  test('Test convertToPath unknown element', async function ({ page }) {
    const { convertToPath } = utilities;
    const attrs = {
      fill: 'red',
      stroke: 'white',
