...
test.describe('draw.Drawing', function () {
  ...
  test('Test draw module', async function ({ page }) {
    assert.ok(draw);
  ...
  test('Test document creation', async function ({ page }) {
    let doc;
    try {
    ...
    try {
      ...
      assert.ok(
        false,
        'Could not create document from valid <svg> element: ' + e
      );
    }
  });

  test('Test nonce', async function ({ page }) {
  ...
  test('Test getId() and getNextId() without nonce', async function ({ page }) {
  ...
  test('Test getId() and getNextId() with prefix without nonce', async function ({
    page,
  }) {
    const prefix = 'Bar-';
  ...
  test('Test getId() and getNextId() with nonce', async function ({ page }) {
  ...
  test('Test getId() and getNextId() with prefix with nonce', async function ({
    page,
  }) {
    const PREFIX = 'Bar-';
  ...
  test('Test releaseId()', async function ({ page }) {
    const doc = new draw.Drawing(svg);
  ...
  test('Test getNumLayers', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
    assert.equal(
      typeof drawing.getNumLayers,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test hasLayer', async function ({ page }) {
    setupSVGWith3Layers(svg);
    const drawing = new draw.Drawing(svg);
    drawing.identifyLayers();

    assert.equal(
      typeof drawing.hasLayer,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test identifyLayers() with empty document', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
  ...
  test('Test identifyLayers() with some layers', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
    setupSVGWith3Layers(svg);
    ...
    assert.equal(
      drawing.all_layers[0].getGroup().getAttribute('class'),
      LAYER_CLASS
    );
    assert.equal(
      drawing.all_layers[1].getGroup().getAttribute('class'),
      LAYER_CLASS
    );
    assert.equal(
      drawing.all_layers[2].getGroup().getAttribute('class'),
      LAYER_CLASS
    );
  ...
  test('Test identifyLayers() with some layers and orphans', async function ({
    page,
  }) {
    setupSVGWith3Layers(svg);
    ...
    assert.equal(
      drawing.all_layers[3].getGroup().getAttribute('class'),
      LAYER_CLASS
    );
  ...
  test('Test getLayerName()', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
    setupSVGWith3Layers(svg);
  ...
  test('Test getCurrentLayer()', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
    setupSVGWith3Layers(svg);
    drawing.identifyLayers();
    ...
    assert.equal(
      typeof drawing.getCurrentLayer,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test setCurrentLayer() and getCurrentLayerName()', async function ({
    page,
  }) {
    const drawing = new draw.Drawing(svg);
    setupSVGWith3Layers(svg);
    ...
    assert.equal(
      typeof drawing.setCurrentLayer,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test setCurrentLayerName()', async function ({ page }) {
    const mockHrService = {
      changeElement() {
        // empty
      },
    ...
    assert.equal(
      typeof drawing.setCurrentLayerName,
      typeof function () {
        /* empty fn */
      }
    );
    ...
    assert.equal(
      oldName,
      mockHrService.changeElement.getCall(0).args[1]['#text']
    );
    assert.equal(
      newName,
      mockHrService.changeElement.getCall(0).args[0].textContent
    );

    cleanupSVG(svg);
  });

  test('Test createLayer()', async function ({ page }) {
    const mockHrService = {
      startBatchCommand() {
        /* empty fn */
      },
  ...
  test('Test mergeLayer()', async function ({ page }) {
    const mockHrService = {
      startBatchCommand() {
        /* empty fn */
      },
    ...
    assert.ok(mockHrService.endBatchCommand.calledOnce);
    assert.equal(
      mockHrService.startBatchCommand.getCall(0).args[0],
      'Merge Layer'
    );
  ...
  test('Test mergeLayer() when no previous layer to merge', async function ({
    page,
  }) {
    const mockHrService = {
      startBatchCommand() {
  ...
  test('Test mergeAllLayers()', async function ({ page }) {
    const mockHrService = {
      startBatchCommand() {
        /* empty fn */
      },
    ...
    assert.equal(
      mockHrService.startBatchCommand.getCall(1).args[0],
      'Merge Layer'
    );
    assert.equal(
      mockHrService.startBatchCommand.getCall(2).args[0],
      'Merge Layer'
    );
  ...
  test('Test cloneLayer()', async function ({ page }) {
    const mockHrService = {
      startBatchCommand() {
        /* empty fn */
      },
  ...
  test('Test getLayerVisibility()', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
    setupSVGWith3Layers(svg);
    drawing.identifyLayers();
    ...
    assert.equal(
      typeof drawing.getLayerVisibility,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test setLayerVisibility()', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
    setupSVGWith3Layers(svg);
    drawing.identifyLayers();
    ...
    assert.equal(
      typeof drawing.setLayerVisibility,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test getLayerOpacity()', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
    setupSVGWith3Layers(svg);
    drawing.identifyLayers();
    ...
    assert.equal(
      typeof drawing.getLayerOpacity,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test setLayerOpacity()', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
    setupSVGWith3Layers(svg);
    drawing.identifyLayers();
    ...
    assert.equal(
      typeof drawing.setLayerOpacity,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test deleteCurrentLayer()', async function ({ page }) {
    const drawing = new draw.Drawing(svg);
    setupSVGWith3Layers(svg);
    drawing.identifyLayers();
  ...
  test('Test svgedit.draw.randomizeIds()', async function ({ page }) {
