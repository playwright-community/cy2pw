...
test.describe('path', function () {
  ...
  test('Test svgedit.path.replacePathSeg', async function ({ page }) {
  ...
  test('Test svgedit.path.Segment.setType simple', async function ({ page }) {
    const path = document.createElementNS(NS.SVG, 'path');
    path.setAttribute('d', 'M0,0 L10,11 L20,21Z');
  ...
  test('Test svgedit.path.Segment.setType with control points', async function ({
    page,
  }) {
  ...
  test('Test svgedit.path.Segment.move', async function ({ page }) {
    const path = document.createElementNS(NS.SVG, 'path');
    path.setAttribute('d', 'M0,0 L10,11 L20,21Z');
  ...
  test('Test svgedit.path.Segment.moveCtrl', async function ({ page }) {
    const path = document.createElementNS(NS.SVG, 'path');
    path.setAttribute('d', 'M0,0 C11,12 13,14 15,16 Z');
  ...
  test('Test svgedit.path.convertPath', async function ({ page }) {
    unitsInit({
      getRoundDigits() {
        return 5;
      },
