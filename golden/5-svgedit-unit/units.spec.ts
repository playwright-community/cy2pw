...
test.describe('units', function () {
  ...
  test('Test svgedit.units package', async function ({ page }) {
    assert.ok(units);
    assert.equal(typeof units, typeof {});
  });

  test('Test svgedit.units.shortFloat()', async function ({ page }) {
    assert.ok(units.shortFloat);
    assert.equal(
      typeof units.shortFloat,
      typeof function () {
        /* empty fn */
      }
    );
  ...
  test('Test svgedit.units.isValidUnit()', async function ({ page }) {
    assert.ok(units.isValidUnit);
    assert.equal(
      typeof units.isValidUnit,
      typeof function () {
        /* empty fn */
      }
    );
    ...
    assert.equal(
      isValidUnit('id', 'uniqueId', document.getElementById('uniqueId')),
      true
    );
    assert.equal(
      isValidUnit('id', 'newId', document.getElementById('uniqueId')),
      true
    );
    assert.equal(isValidUnit('id', 'uniqueId'), false);
    assert.equal(
      isValidUnit('id', 'uniqueId', document.getElementById('nonUniqueId')),
      false
    );
  });

  test('Test svgedit.units.convertUnit()', async function ({ page }) {
    assert.ok(units.convertUnit);
    assert.equal(
      typeof units.convertUnit,
      typeof function () {
        /* empty fn */
      }
    );
