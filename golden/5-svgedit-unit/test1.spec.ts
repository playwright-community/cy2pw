...
test.describe('Basic Module', function () {
  // helper functions
  /*
  ...
  */
  ...
  test('Test existence of SvgCanvas object', async function ({ page }) {
    assert.equal(typeof {}, typeof svgCanvas);
  });

  test.describe('Path Module', function () {
    test('Test path conversion from absolute to relative', async function ({
      page,
    }) {
      ...
      assert.equal(
        seglist.numberOfItems,
        4,
        'Number of segments before conversion'
      );

      // verify segments before conversion
      let curseg = seglist.getItem(0);
      assert.equal(
        curseg.pathSegTypeAsLetter.toUpperCase(),
        'M',
        'Before conversion, segment #1 type'
      );
      curseg = seglist.getItem(1);
      assert.equal(
        curseg.pathSegTypeAsLetter.toUpperCase(),
        'L',
        'Before conversion, segment #2 type'
      );
      curseg = seglist.getItem(3);
      assert.equal(
        curseg.pathSegTypeAsLetter.toUpperCase(),
        'Z',
        'Before conversion, segment #3 type' + dAbs
      );
      ...
      assert.equal(
        d,
        'm100,100l100,0l-100,0z',
        'Converted path to relative string'
      );
  ...
  test.describe('Import Module', function () {
    test('Test import use', async function ({ page }) {
      svgCanvas.setSvgString(
    ...
    test('Test invalid attribute', async function ({ page }) {
      svgCanvas.setSvgString(
      ...
      assert.equal(
        t.getAttribute('d'),
        null,
        'Imported a <text> with a d attribute'
      );
    });

    // This test makes sure import/export properly handles namespaced attributes
    test('Test importing/exporting namespaced attributes', async function ({
      page,
    }) {
      ...
      const attrVal = document
        .getElementById('se_test_elem')
        .getAttributeNS('http://svg-edit.googlecode.com', 'foo');

      assert.strictEqual(
        attrVal,
        'bar',
        true,
        'Preserved namespaced attribute on import'
      );

      const output = svgCanvas.getSvgString();
      const hasXlink = output.includes(
    ...
    test('Test import math elements inside a foreignObject', async function ({
      page,
    }) {
      ...
      assert.equal(
        math.nodeName,
        'math',
        'Math element has the proper nodeName'
      );
      assert.equal(math.getAttribute('id'), 'm', 'Math element has an id');
      assert.equal(
        math.namespaceURI,
        'http://www.w3.org/1998/Math/MathML',
        'Preserved MathML namespace'
      );
    });

    test('Test importing SVG into existing drawing', async function ({ page }) {
    ...
    test('Test importing SVG remaps IDs', async function ({ page }) {
      /* const doc = */ svgCanvas.setSvgString(
        '<svg width="640" height="480" xmlns="http://www.w3.org/2000/svg">' +
          '<g><title>Layer 1</title>' +
      ...
      assert.notEqual(
        circles.item(0).getAttribute('fill'),
        'url(#svg_2)',
        'Circle fill value not remapped'
      );
      assert.notEqual(
        rects.item(0).getAttribute('stroke'),
        'url(#svg_2)',
        'Rectangle stroke value not remapped'
      );
